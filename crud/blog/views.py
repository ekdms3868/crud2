...
#글쓰기 함수
# def create(request): #create함수 선언
#     # post = Blog()
...
#         form = PostForm(request.POST) # form은 PostForm(request.POST)
#         if form.is_valid(): #만약 form.is_valid가 true면
#             form = form.save(commit=False) #form은 form의 save에서 저장된 것이고
#             form.pub_date = timezone.datetime.now()#form의 날짜와 시간은 현재시간을 저장하고
#             form.save() #변경된 사항을 모두 저장하고
#             return redirect('main') #main페이지로 돌아간다.
#     else:
#         form = PostForm #만약 POST방식이 아니라면 form에 PostForm을 저장한다.
#         return render(request,'blog/write.html',{'form':form}) # 그리고 form을 return 한다.
...
def search(request, hashtag_id):
    ...
    #     if form.is_valid():
    #         comment = form.save(commit=False)
    #         comment.hashtag_id = hashtag
    #         comment.text = form.cleaned_data['text']
    #         comment.save()
    #         return redirect('hashtag',id)
    # else:
    #     form = HashtagForm()
    #     return render(request, 'blog/search.html', {'hashtag':hashtag})
